...
const salesRows = [
  ...
  { id: "#2402", customer: "TechStart", product: "Sensor Kit", amount: "$890", status: "Processing" },
...
const CountUp = ({ target, prefix = "" }: { target: number; prefix?: string }) => {
  ...
  return (
    <span ref={ref} className="font-mono text-xl sm:text-2xl font-bold text-foreground">
  ...
  <section id="preview" className="py-20 sm:py-32">
    <div className="container mx-auto px-4 sm:px-6">
      ...
      <motion.div
        ...
        className="glass-card p-4 sm:p-6 lg:p-8 space-y-6 overflow-hidden"
      >
        {/* Stats counters */}
          ...
          <div className="glass-card p-4 min-w-0">
            ...
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={barData}>
                ...
                <YAxis tick={{ fill: "hsl(215 20% 65%)", fontSize: 11 }} axisLine={false} tickLine={false} width={40} />
                <Tooltip contentStyle={{ background: "hsl(220 40% 13%)", border: "1px solid hsl(217 33% 20%)", borderRadius: 8, color: "hsl(210 40% 98%)" }} />
              ...
              <table className="w-full text-sm min-w-[360px]">
                <thead>
                  <tr className="text-left text-muted-foreground border-b border-border/50">
                    ...
                    <th className="pb-2 pr-3 font-medium hidden sm:table-cell">Product</th>
                ...
                <tbody>
                  {salesRows.map((row) => (
                    <tr key={row.id} className="border-b border-border/30">
                      ...
                      <td className="py-2.5 pr-3 text-foreground max-w-[80px] truncate">
                        {row.customer}
                      </td>
                      <td className="py-2.5 pr-3 text-muted-foreground max-w-[100px] truncate hidden sm:table-cell">
                        {row.product}
                      </td>
                      <td className="py-2.5 pr-3 font-mono text-foreground whitespace-nowrap">
                        {row.amount}
                      </td>
                      <td className="py-2.5">
